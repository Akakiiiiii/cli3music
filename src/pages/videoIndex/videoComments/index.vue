<template>
  <div class="video-com">
    <div class="titleFixed">
      ...
      <video-card :videoId="videoId"
                  :allTime="allTime"
                  @blockNav="blockNav"
                  @hideNav="hideNav"></video-card>
    </div>
    <div v-show="!load"
         class="bottom-wra"
         ref="wra"
         @scroll="scroll">
      <video-info class="pd23"
                  :title="title"
                  :playTime="playTime"
                  ...
                  :videoGroup="videoGroup.slice(0,3)"></video-info>
      <video-creator :class="{creatorFixed}"
                     :avatarUrl="avatarUrl"
                     :nickname="nickname"></video-creator>
      <div class="split"
           :style="{marginTop: top}"></div>
      <h1 class="pd23 title">相关视频</h1>
      <video-list class="pd23"
      ...
      <div class="split"></div>
      <comments v-show="hotComments.length"
                title="精彩评论"
                :comments="hotComments"
                class="pd23"
                @showMenu="showMenu"
                @likeComment="likeComment"></comments>
      <comments v-show="comments.length"
                title="最新评论"
                :total="total"
                :comments="comments"
                class="pd23"
                @showMenu="showMenu"
                @likeComment="likeComment"></comments>
      <van-search class="border-top commentInp"
                  v-model="input"
        ...
        <div slot="action"
             :class="{disable: !input}"
             @click="pushCom(input)">发送</div>
      </van-search>
      <center-menu @update:show="updateShow"
                   @delCom="delCom"
                   :isShow="isShow"></center-menu>
    </div>
